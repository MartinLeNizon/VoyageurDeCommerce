...
int computeD(int i, set s, int n, int** cost){
    ...
    // for (int i=0; i<length_memD; i++) {
    //     for (int j=0; j<width_memD; j++) {
    ...
    for (int j=1; j<n; j++){
        if (isIn(j,s)){
            compteur++;
            int d;
            if ( (d = memD[j][removeElement(s,j)]) == 0) {
                d = computeD(j, removeElement(s,j), n, cost);
                memD[j][removeElement(s,j)] = d;
            }
...
int main(){
    ...
    memD = (int**) malloc(length_memD*sizeof(int*));

    for (int i=0; i<length_memD; i++) {
        memD[i] = (int*) malloc(width_memD*sizeof(int));
        for (int j=0; j<width_memD; j++) {
            memD[i][j] = 0;
        }
    }
    ...
    printf("Nombre de passage total : %d \n", compteur);
